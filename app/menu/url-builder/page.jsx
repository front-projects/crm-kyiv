  ...
  return (
    <section className="w-2/3 h-full py-10 max-sm:w-[95vw]">
      <form className="flex flex-col gap-4 w-full">
        ...
        <div className='bg-[#131a22] p-2 rounded-md'>
            Meta
        </div>
        <h3 className='text-gray-600 text-sm mb-[-10px]'>Creative Headline (Requried)</h3>
        
        <input 
        value={creativeHeadline}
        onChange={(e)=> setCreativeHeadline(e.target.value)}
        type="text" className='p-2 bg-[#131a22] rounded-md text-white' placeholder='Crative Headline*' />

        <h3 className='text-gray-600 text-sm mb-[-10px]'>Additional Options:</h3>

        <div>
            <FormControlLabel control={<Checkbox color='secondary' onChange={()=> setShowDynamic(!showDynamic)} sx={{color:'white'}}/>}  label="Dynamic Terms"/>
        </div>

        {showDynamic && 
        <div className='bg-[#131a22] p-4 rounded-md w-full flex items-center gap-4 flex-wrap'>
            {dynamic?.map((el,index)=> <DynamicItem key={el+index} item={el} onUpdate={(upd)=> updateDynamic(upd,index)} onRemove={()=> removeDynamic(index)}/>)}
            <div className='flex items-center gap-2 w-full'>
            <input 
            onChange={(e)=> setDynamicInput(e.target.value)}
            value={dynamicInput}
            type="text" className='bg-transparent text-white border-2 rounded-md p-2' placeholder='Dynamic Terms'/>
            <div className='text-purple-600 cursor-pointer text-[200%]' onClick={submitAdding}><FaCirclePlus/></div>
            </div>
        </div>
        }
        ...
        <h3 className='text-gray-600 text-sm mb-[-10px]'>Your automatically generated URL</h3>
        ...
        className="w-full p-4 border-2 cursor-pointer hover:border-purple-600 border-gray-400 rounded-md flex items-center justify-between">
            ...
            {isCopied ? <div>Copied</div> : <div className='text-[150%] hover:text-purple-600'>
                <FaCopy/>
            </div>}
